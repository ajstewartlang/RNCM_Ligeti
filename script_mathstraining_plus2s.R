...
colnames(raw_data) <- c("ID", "maths_training", "start", names_list)
...
# Plot histogram removing presses 
tidy_data_filtered %>%
  ggplot(aes(x = time)) +
...
tidy_data$maths_training <- as.integer(tidy_data$maths_training)
tidy_data_filtered$maths_training <- as.integer(tidy_data_filtered$maths_training)

tidy_data_filtered %>%
  mutate(time = as.integer(value)/1000) %>%
  ggplot(aes(x = time, y = reorder(ID, maths_training), colour = maths_training)) +
  geom_point(alpha = .2, size = 5) +
  theme(axis.text.y = element_blank()) +
  labs(x = "Time", y = NULL) +
  guides(colour = FALSE) +
  geom_vline(xintercept = 7.11) +
...
maths_data <- tidy_data_filtered %>%
  select(c(ID, maths_training)) %>%
  distinct(ID, .keep_all = TRUE)

my_df <- left_join(my_df, maths_data , by = "ID")
